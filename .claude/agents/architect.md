...
4. **If the design covers a moving or animated visual entity** (a character, vehicle, or creature — not a static prop), **design its spatial orientation as its own concern**, separate from which animation clip/pose plays (project convention, added after Sprint 4's retro): does it rotate to face its direction of travel or target, and where/how is that computed? ADR 0015 designed the farmer's `AnimationMixer` lifecycle and clip crossfading in real detail but never addressed rotation — the resulting "never turns to face its movement" bug went undesigned, then unimplemented, then uncaught through code review and two live-verification passes, until a human drove the game live and noticed by eye.
5. Produce a design doc, not code.
