...
- Add a river and mountains that establish the farm's sense of place and give the terrain's edges a visual identity ("terrain features" half; ~~mountains double as the answer to `drive-terrain-and-gas.md` Open Question 3 — a visible horizon rather than an invisible wall~~ **note (2026-07-10):** per AC3a, mountains are no longer a boundary/horizon treatment — that reopens `drive-terrain-and-gas.md` Open Question 3 as genuinely unresolved again, not a blocker for this issue, but flagged here so it isn't silently forgotten).
...
- **Mountains → non-collidable backdrop, placed beyond the drivable terrain bounds.** *(terrain features)* ~~Never reachable, never blocks the truck's path; visible from anywhere in the drivable area as a horizon feature. This is the recommended resolution to `drive-terrain-and-gas.md` Open Question 3 (soft boundary should have a visible edge given the farm setting) — the mountains *are* that visible edge treatment.~~ **Superseded 2026-07-10 (human decision, mid-Sprint-4-implementation):** while the sourced mountain models' backdrop treatment shipped and rendered correctly, the human decided during review that they want one large, reachable, collidable mountain landmark placed *inside* `TERRAIN_BOUNDS` instead — see the new AC3a below. The original backdrop-ring approach (12 instanced small mountains outside bounds) is removed, not kept alongside the landmark.
...
- ~~**AC3 (mountains as backdrop):** Given the drivable terrain bounds (`TERRAIN_BOUNDS`), when mountains are rendered, then they sit outside those bounds, are visible as a horizon/edge feature from anywhere inside the drivable area, and are never collidable and never reachable by the truck.~~ **Superseded 2026-07-10 — see AC3a.**
- **AC3a (single reachable landmark mountain, supersedes AC3, 2026-07-10):** Given the drivable terrain bounds, when the scene is rendered, then exactly one large mountain is placed *inside* `TERRAIN_BOUNDS` (not outside), positioned clear of every existing structure/obstacle/the truck's start position. It is a solid, always-collidable landmark — same "unconditional solid, blocks regardless of wheel tier, no damage/fail" behavior as the windmill/barn/farmhouse (AC2) — reusing the `StructureInstance`/`createStructureColliders` pattern rather than inventing a new one. Target rendered height ≈ 3× the barn's rendered height (per `docs/architecture/0012-environment-dressing-and-terrain-features.md`'s Sprint-4 addendum for the exact derivation). The prior 12-instance non-collidable backdrop ring is removed, not kept alongside this landmark — this is a replacement, not an addition.
- **AC4 (river is decorative, non-blocking):** Given a river placed on or near the terrain, when the truck drives across or along it, then truck movement, top speed, gas drain/regen, and coin systems are completely unaffected — the river has zero mechanical effect this sprint. **Unaffected by the AC3a change** — the river's design and implementation stand as originally shipped.
