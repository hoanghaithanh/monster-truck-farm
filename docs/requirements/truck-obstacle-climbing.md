...
Status: **Direction resolved by the human (2026-07-09): approach (a), lightweight visual approximation** (see Open Question 1). Still not ready to pull into a sprint as-is — the architect still needs to design the actual mechanism (raycast/height-sample strategy, curve shape, per-obstacle tuning approach) before this can be sized/estimated; the human's answer sets the direction, not the design.
...
2. **(Non-blocking — architect's design question) Mechanism.** The human's answer sets *direction*, not the specific technique. The architect still needs to design: how the obstacle footprint/height is sampled per wheel or per truck-center, what curve shape the lift/tilt follows, how it's tuned per obstacle class/wheel tier, and how it integrates with `setTruckTransform()`/`buildTruckRig()` (see `truck-wheel-motion.md`'s ADR, which the architect should design alongside this one — both touch the same render-layer per-frame update path). This doc's AC set (AC1-AC6) remains the acceptance bar regardless of the exact mechanism chosen.
3. **(Non-blocking) Sprint sizing.** Now that approach (a) is confirmed, this is plausibly a single sprint's worth of work (visual/animation layer, existing architecture, no foundational physics change) — but still needs the architect's design pass before a real size estimate, since the exact mechanism (and whether it needs new per-obstacle authored data) isn't decided yet.
