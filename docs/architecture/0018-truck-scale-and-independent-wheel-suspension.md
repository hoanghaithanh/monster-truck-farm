...
**Addendum (2026-07-12, ADR 0020 pointer):** the truck-body-lift feature (issue #65, ADR 0020) raises the static body-to-wheel baseline gap that this `maxTravel` travels *within* — the shared tunable between the two ADRs. The lift shrinks each wheel's retained fender overlap to `(1 − BODY_LIFT_FACTOR)·r`, and this `maxTravel = 0.25` exceeds that overlap on the two smaller body tiers, so a full *down*-travel could un-tuck the wheel from the fender (ADR 0020 AC5 "detach/hole"). This is the Sprint-1-retro class of interaction (two individually-reasonable numbers from separate ADRs). Reconciled in ADR 0020 §4: `maxTravel` should come down to ≈0.15 (or become proportional, `m·r`), respecting `maxTravel ≲ (1 − BODY_LIFT_FACTOR)·r_min`. If `maxTravel` is retuned here later, re-check it against ADR 0020's lift factor, and vice versa.
