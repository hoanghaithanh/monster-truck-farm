...
## Addendum (2026-07-10) — Mountain redesign: reachable landmark supersedes backdrop ring

Mid-Sprint-4 implementation of issue #47, after the backdrop-ring design above had already shipped and rendered correctly, the human decided they want a single large, reachable, collidable mountain landmark placed *inside* `TERRAIN_BOUNDS` instead of (not in addition to) the 12-instance non-collidable backdrop ring outside bounds. See `docs/requirements/environment-dressing.md` AC3a (supersedes AC3).

**What changes:**
- Remove the backdrop-ring data/rendering (the `MountainInstance`/`STUB_MOUNTAINS` ring-generation code from the original #47 implementation) entirely — this is a replacement, not an addition.
- The single landmark mountain becomes a `StructureInstance` with `kind: 'mountain'`, `collidable: true` — reusing the exact same `createStructureColliders`/`AssetRegistry`/`UpgradableObject`/keep-out plumbing §1/§2/§5 already built for windmill/barn/farmhouse, rather than inventing a parallel system. `StructureKind` widens to `'windmill' | 'barn' | 'farmhouse' | 'mountain'`.
- `structureKeepouts()` needs no code change — it already filters on `collidable`, not a hardcoded kind list, so the mountain gets spawn keep-out for free the moment it's a `collidable: true` `StructureInstance`.
- The river is unaffected by this addendum — still procedural, still non-collidable, still excluded from keep-out, per §3 as originally designed.

**Sizing (target rendered height ≈ 3× the barn):** `buildStructureDisplayModel(source, targetWidth)` (§2, `render/scene.ts`) scales a structure by *width* (`footprintRadius * 2`), not height — it derives whatever height falls out of the raw model's own aspect ratio. The barn's own rendered height is therefore not a free parameter; measured directly from the shipped asset: raw barn bounding box is (7.73, 7.01, 8.22) on (x,y,z), `footprintRadius = 3` → `targetWidth = 6` → `scaleFactor = 6 / 7.73 ≈ 0.777` → rendered height `≈ 7.01 × 0.777 ≈ 5.44` units. **3× that ≈ 16.3 units** is the mountain's target rendered height — a genuinely large landmark (>40% of the terrain's 40-unit span), consistent with "a big visible feature to drive toward," not a small hill.

Because `buildStructureDisplayModel` sizes by width and this spec is a height target, the developer implementing this needs either (a) a height-driven sizing variant (mirroring how `buildChickenDisplayModel` already targets height via `CHICKEN_TARGET_HEIGHT` rather than width), deriving the resulting `footprintRadius` from the mountain model's own raw aspect ratio at that target height, or (b) computing an equivalent `targetWidth` from the mountain model's own raw width/height ratio and reusing the existing width-driven function unchanged. Either is architecturally fine — this addendum specifies the target number, not the code path to hit it.
