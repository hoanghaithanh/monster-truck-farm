...
Workflow that worked well and should repeat for the remaining art backlog (chicken model #28, farmer model #29, environment dressing #26 — windmill/barn/farmhouse/river/mountains):
...
2. Render candidates locally (a small Puppeteer + Three.js `GLTFLoader` scratch script, or the model's own hosted preview if the source site provides one, e.g. poly.pizza's `modelviewer.dev` embed URLs) and actually look at them before picking — don't commit to an asset sight-unseen.
...
5. **Always take live screenshots and actually look at them** — both the developer's own verification pass and the orchestrator's before reporting to the human. This session caught a floating decal, an invisible decal, a muddy cosmetic tint, and hollow-looking wheels this way — none of them were caught by unit tests or code review alone, and one wasn't caught by the implementing agent's own first screenshot pass either.
